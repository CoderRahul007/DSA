...
class Solution(object):
    def wordBreak(self, s, wordDict):
        ...
        return self.dp(len(s),wordDict)
        
    def dp(self, i, wordDict):
        if i in self.memo:
            return self.memo[i]
        if i==0:
            return True
        for j in range(i):
            if self.s[j:i] in wordDict:
                if self.dp(j,wordDict) == True:
                    self.memo[i] = True
                    return True
        self.memo[i] = False
        return False
...
# is I also keep a set of possible word lengths to reduce look ahead possibilities and avoid needing to build new strings.
