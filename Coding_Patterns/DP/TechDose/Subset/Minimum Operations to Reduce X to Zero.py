...
# first find the maxlen of subarrray whose sum is equal to a Sum K
# then check if the total sum of all elements are equal to the X it means we have to remove all the elements so ans is len(arr)
# else we will find the maxlen of subarrray whose sum is equal to sum(arr) - x
# if its 0 then no ans else ans is n-k

# [1,1,4,2,3], x = 5 total = 11
# total != X so get the maxlen of subarrray whose sum is equal to sum(arr) - x ( 11 - 5)
# which is 3 ( 1 , 1 , 4)
# now 5 - 3 = 2