...
def hasSubset(set , n , sum):
        ...
        if n == 0 and sum != 0:
            return False
        if set[n-1] > sum:  #If last element is greater than sum, then ignore it  , to remove negative sum
...
def hasSubsetDP(set , n , sum):
    # The value of subset[i][s] will be true if there is a 
    # subset of set[0..s-1] with sum equal to i 

    subset = [[-1 for s in range(sum + 1)] for i in range(n + 1)]
    ...
    for i in range(1, n + 1):
        for s in range(1, sum + 1):
            if s < set[i-1]:  # if less then dont include
                subset[i][s] = subset[i-1][s]
            if s >= set[i-1]: # both option exclude and include
                subset[i][s] = subset[i-1][s] or subset[i-1][s-set[i-1]]
    return subset[n][sum]   

set = [3, 34, 4, 12, 5, 2]
sum = 13
n = len(set)
mem = [[-1 for s in range(sum + 1)] for i in range(n+1)]
