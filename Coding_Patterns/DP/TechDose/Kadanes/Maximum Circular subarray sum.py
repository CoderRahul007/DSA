...
def maxCircularSumComplete(a, n):
	...
	# boundary casse if all elements are negative  
	# then min_so_far will be sum so give the max_so_far means highest negative number		
	if (min_so_far == sum(a)): # all are negative if we add all the negative intger
		# they will be most minimum and hense the sum of all neg array
		return max_so_far
		# we want the maximum so return the max_so_far
