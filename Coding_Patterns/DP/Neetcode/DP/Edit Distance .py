...
def editDistanceRecur(str1, str2, m, n):
    if m == 0:
        return n # all str2 char need to add so n 
    if n == 0:
        return m # all str1 char need to delete so m
    ...
    return 1 + min(editDistanceRecur(str1, str2, m, n-1),    # Insert we have inserted a char in str1 so m pointer will be same but n pointer will decrease
                   editDistanceRecur(str1, str2, m-1, n),    # Remove we have deleted a char in str1 so m pointere will decrese but n pointer wont
                   editDistanceRecur(str1, str2, m-1, n-1)    # Replace we have replaced a char in both str1 and str2 so both pointere will decrease
                   )
