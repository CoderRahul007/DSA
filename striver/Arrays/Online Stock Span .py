...
#  for a stock and we need to calculate the span of the stock’s price for all n days. The span
#  Si of the stock’s price on a given day i is defined as the maximum number of consecutive days
#  just before the given day, for which the price of the stock on the current day is less than its
...
def findSpans(price):
    ...
    for i in range(n):
        ...
        while( len(stk) and price[stk[- 1]] <= price[i]):
            stk.pop()
